...
const convertToCSV = () => {
  ...
  xlsFiles.forEach(item => {
    ...
    fs.writeFile(justName + ".csv", writeStr, function (err) {
      ...
      console.log(justName + " was saved in the current directory!");
    });
...
const downloadFile = async () => {
  const file = fs.createWriteStream("TaxSale349.xls");
  await http.get("http://maps.wycokck.org/gisdata/taxsale/TaxSale349.xls", function (response) {
    response.pipe(file);
